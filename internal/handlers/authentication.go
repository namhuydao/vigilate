...
func (repo *DBRepo) LoginScreen(w http.ResponseWriter, r *http.Request) {
	...
	err := helpers.TemplateRender(w, r, "./views/templates/", "login", helpers.TemplateData{})
	if err != nil {
...
func (repo *DBRepo) Login(w http.ResponseWriter, r *http.Request) {
	...
	id, err := repo.DB.Authenticate(r.Form.Get("email"), r.Form.Get("password"))
	if errors.Is(err, models.ErrInvalidCredentials) {
		app.Session.Put(r.Context(), "error", "Invalid credentials")
		err := helpers.TemplateRender(w, r, "./views/", "login", helpers.TemplateData{Error: "error"})
		if err != nil {
			printTemplateError(w, err)
		}
		return
	...
	if r.Form.Get("remember") == "remember" {
		...
		err = repo.DB.InsertRememberMeToken(int(id), sha)
		if err != nil {
			log.Println(err)
		}
	...
	u, err := repo.DB.GetUserById(int(id))
	if err != nil {
		log.Println(err)
		ClientError(w, r, http.StatusBadRequest)
		return
...
func (repo *DBRepo) Logout(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if len(key) > 0 {
			...
			hash := split[1]
			err = repo.DB.DeleteRememberMeToken(hash)
			if err != nil {
				log.Println(err)
			}
		}
